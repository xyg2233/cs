...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def _fix_pic(self, pic_url):
		...
		return pic_url + "@Referer=https://api.douban.com/"
	# ============================================
	...
	def homeVideoContent(self):
		...
		for item in joList:
			...
			# 最小改动：加.get保护 + 套_fix_pic
			pic = item.get('pic', {}).get('normal', '') if item.get('pic') else ''
			lists.append({
	...
	def categoryContent(self,tid,pg,filter,extend):		
		...
		for vod in jolist:
			if vod.get("type", "") == "movie" or vod.get("type", "") == "tv":
				...
				# 最小改动：加isinstance保护 + 套_fix_pic
				pic_obj = vod.get("pic", "")
				pic = pic_obj.get("normal", "") if isinstance(pic_obj, dict) else ""
				title = vod.get("title", "")
				videos.append({
