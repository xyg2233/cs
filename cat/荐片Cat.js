...
async function init(cfg) {
    try {
        // ✅ 优先使用 ext.sites 或 ext.host
        let host = cfg.ext?.sites?.trim() || cfg.ext?.host?.trim() || 'https://ev2089.zxbwv.com';
        HOST = host.replace(/\/$/, '');
        ...
        // ✅ 关键：优先使用 ext.imgUrl
        let imgHost = cfg.ext?.imgUrl?.trim() || '';
        
        // 如果 ext 没给 imgUrl，再尝试从接口获取
        if (!imgHost) {
            const batchPathList = ['/api/appAuthConfig'];
            const resObj = safeParseJSON(await request(`${HOST}${batchPathList[0]}`));
            imgHost = resObj?.data?.imgDomain?.trim() || '';
        }
        
        // ✅ 确保协议正确
        if (imgHost && !/^http/.test(imgHost)) {
        ...
        // 预加载分类数据
        const batchPathList = ['/api/term/home_fenlei', '/api/dyTag/hand_data?category_id=88'];
...
async function home(filter) {
    try {
        ...
        let classes = typeObj.map((item) => { 
            return {
                type_name: item?.name ?? '分类名', 
                type_id: item?.id?.toString() ?? '分类值'
            }; 
        }).slice(1);
        ...
        try {
            ...
            fclasses.forEach((it, idx) => {
                ...
                filters[it.type_id] = filterObj.map(flt => {
                    ...
                    let kval = flt.data ?? [];
                    let kvalue = kval.map(item => { 
                        return { n: item?.name || '未知', v: item?.id ?? '' }; 
                    });
...
async function category(tid, pg, filter, extend) {
    try {
        ...
        let cateUrl = (tid === '99') 
            ? `${HOST}/api/dyTag/list?category_id=${tid}` 
            : `${HOST}/api/crumb/${aPath}?fcate_pid=${tid}&category_id=${extend?.cateId ?? ''}&type=${extend?.class ?? ''}&area=${extend?.area ?? ''}&year=${extend?.year ?? ''}&sort=${extend?.by ?? ''}&page=${pg}`;
...
async function detail(id) {
    try {
        ...
        if (kflag === '短剧') {
            ...
            const arrToStr = (arr, str) => { 
                return (arr ?? []).map(it => (it.name || it.value)?.trim() || '').join('/') || str; 
            };
...
function getVodList(listArr) {
    try {
        let kvods = [];
        listArr.forEach(it =>{
            let isDj = Array.isArray(it.categories) && it.categories.includes(67) || 
                      Array.isArray(it.res_categories) && it.res_categories.some(item => item.name === '短剧' && item.id === 67);
            let vflag = isDj ? '短剧' : '其它';
            let vName = it.title || '名称';
            
            // ✅ 智能处理图片URL
            let rawPic = it.path || it.cover_image || it.thumbnail || '';
            let vPic = '';
            ...
            if (rawPic.startsWith('http://') || rawPic.startsWith('https://')) {
                ...
                // 相对路径，拼接域名
                let baseHost = KParams.imgHost || HOST;
                if (baseHost && !baseHost.endsWith('/') && !rawPic.startsWith('/')) {
                    vPic = baseHost + '/' + rawPic;
                } else if (baseHost && baseHost.endsWith('/') && rawPic.startsWith('/')) {
                    vPic = baseHost + rawPic.substring(1);
                } else {
                    vPic = baseHost + rawPic;
                }
            }
